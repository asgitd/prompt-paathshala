...
async function callClaude(systemPrompt, userMessage) {
  ...
  if (sarvamKey) {
    try {
      const sr = await fetch("https://api.sarvam.ai/v1/chat/completions", {
        method: "POST",
      ...
      if (sr.ok) {
        ...
        if (st) {
          ...
          while (clean.includes("<think>")) {
            const start = clean.indexOf("<think>");
            const end = clean.indexOf("</think>");
            if (end > start) clean = clean.slice(0, start) + clean.slice(end + 8);
            else break;
          }
          // Remove <thinking>...</thinking> blocks
          while (clean.includes("<thinking>")) {
            const start = clean.indexOf("<thinking>");
            const end = clean.indexOf("</thinking>");
            if (end > start) clean = clean.slice(0, start) + clean.slice(end + 11);
            else break;
          }
          clean = clean.trim();
  ...
  if (!apiKey) return "⚠️ API Key nahi mili. Vercel mein VITE_GEMINI_KEY add karo.";
  ...
  for (const model of models) {
    try {
      ...
      if (!response.ok) {
        ...
        if (msg.includes("quota") || msg.includes("RESOURCE_EXHAUSTED") || msg.includes("demand")) continue;
        return "❌ Error: " + (msg || "Dobara try karo.");
      }
...
// Clean AI response - remove <think> tags and fix formatting
...
function PromptEvaluatorTab({ dark }) {
  ...
  const agents = [
    { id: "clarity", name: "Clarity Agent", emoji: "🎯", color: "#3b82f6",
      role: "Tu ek prompt clarity expert hai. User ka prompt dekh aur Hindi mein batao: 1. SCORE: /10 2. KYA SAHI HAI 3. KYA MISSING HAI. Concise raho." },
    { id: "effectiveness", name: "Effectiveness Agent", emoji: "⚡", color: "#f59e0b",
      role: "Tu ek AI output quality expert hai. User ka prompt dekh aur Hindi mein batao: 1. SCORE: /10 2. IS PROMPT SE KAISA OUTPUT AAYEGA 3. KYA ADD KARO. Concise raho." },
    { id: "improved", name: "Improvement Agent", emoji: "🔧", color: "#10b981",
      role: "Tu ek expert prompt engineer hai. User ka prompt lete hue ek improved better version likho. Sirf improved prompt do, explanation mat do. Hinglish mein." }
  ];
  ...
  const evaluate = async () => {
    ...
    await Promise.all(agents.map(async (a) => {
      res[a.id] = await callClaude(a.role, "Yeh prompt evaluate karo: " + prompt);
    }));
    setResults(res);
    setLoading(false);
  };
  ...
  return (
    <div style={{paddingBottom:40}}>
      <div style={{textAlign:"center", marginBottom:20}}>
        ...
        <div style={{fontFamily:"Syne,sans-serif", fontWeight:800, fontSize:20, color:dark?"#e8e8f0":"#111118"}}>Multi-Agent Prompt Evaluator</div>
        <div style={{fontSize:13, color:dark?"#6b7280":"#9ca3af", marginTop:4}}>3 AI agents milke tumhara prompt evaluate karenge!</div>
      </div>
      <div style={{background:dark?"rgba(255,255,255,0.04)":"#f8f8fb", borderRadius:14, padding:16, marginBottom:16, border:"1px solid " + (dark?"rgba(255,255,255,0.08)":"rgba(0,0,0,0.06)")}}>
        <div style={{fontFamily:"Syne,sans-serif", fontWeight:800, fontSize:13, color:"#a5b4fc", marginBottom:10}}>📝 Apna Prompt Yahan Likho:</div>
        <textarea value={prompt} onChange={e=>setPrompt(e.target.value)}
          placeholder="Example: Mujhe essay likhna hai climate change pe..."
          style={{width:"100%", minHeight:100, padding:12, borderRadius:10, border:"1px solid " + (dark?"rgba(255,255,255,0.1)":"rgba(0,0,0,0.1)"), background:dark?"rgba(0,0,0,0.3)":"#fff", color:dark?"#e8e8f0":"#111118", fontFamily:"DM Mono,monospace", fontSize:13, resize:"vertical", outline:"none", boxSizing:"border-box"}} />
        <button onClick={evaluate} disabled={loading||!prompt.trim()}
          style={{width:"100%", marginTop:10, padding:12, borderRadius:12, border:"none", background:"linear-gradient(135deg,#4f46e5,#7c3aed)", color:"#fff", fontFamily:"Syne,sans-serif", fontWeight:800, fontSize:15, cursor:"pointer", opacity:(loading||!prompt.trim())?0.6:1}}>
          {loading ? "⏳ 3 Agents Evaluate kar rahe hain..." : "🚀 Evaluate Karo"}
        </button>
      </div>
      {loading && (
        <div style={{textAlign:"center", padding:"20px 0", color:dark?"#6b7280":"#9ca3af", fontFamily:"DM Mono,monospace", fontSize:13}}>
      ...
      {results && (
        <div>
          {agents.map(a => (
            <div key={a.id} style={{background:dark?"rgba(0,0,0,0.2)":"#fff", border:"1px solid " + a.color + "44", borderLeft:"4px solid " + a.color, borderRadius:12, padding:16, marginBottom:14}}>
              <div style={{display:"flex", alignItems:"center", gap:8, marginBottom:10}}>
                <span style={{fontSize:20}}>{a.emoji}</span>
                <span style={{fontFamily:"Syne,sans-serif", fontWeight:800, fontSize:14, color:a.color}}>{a.name}</span>
              </div>
              <div style={{fontSize:13, color:dark?"#d1d5db":"#374151", lineHeight:1.8, whiteSpace:"pre-wrap"}}>{results[a.id]}</div>
            </div>
          ))}
          <button onClick={()=>{setResults(null);setPrompt("");}}
            style={{width:"100%", padding:10, borderRadius:10, border:"1px solid " + (dark?"rgba(255,255,255,0.1)":"rgba(0,0,0,0.1)"), background:"transparent", color:dark?"#6b7280":"#9ca3af", fontFamily:"DM Mono,monospace", fontSize:12, cursor:"pointer"}}>
...
function UpgradeModal({ onClose, onUpgrade }) {
  ...
  const PLANS = {
    monthly:  { price: 49,  label: "₹49",  name: "Monthly",   desc: "per month",       amount: 4900,  tag: null,           popular: false },
    halfyear: { price: 199, label: "₹199", name: "6 Months",  desc: "₹33/month only!", amount: 19900, tag: "Save 32%",      popular: true  },
    yearly:   { price: 365, label: "₹365", name: "Yearly",    desc: "₹30/month only!", amount: 36500, tag: "Best Value 🔥", popular: false },
  };
  ...
  const handlePayment = () => {
    ...
    const options = {
      ...
      description: "Prompt Paathshala Pro",
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-sub">Monthly, 6 Month, ya Yearly — tumhari marzi!</div>
        <div className="modal-features">
          {[
            ...
            ["✅","Live AI Tester — unlimited"],
            ["✅","Quiz with AI evaluation"],
            ["✅","Kids Zone — Homework, Games, Science"],
...
// Simulated Auth for Preview (Real Firebase in deployed app)
const SimulatedAuth = {
  currentUser: null,
  onAuthStateChanged: (cb) => { cb(null); return () => {}; },
  signInWithPhoneNumber: async (phone) => {
    return { confirm: async (otp) => {
      if (otp === "123456") {
        SimulatedAuth.currentUser = { phoneNumber: phone, uid: "demo_" + Date.now() };
        return { user: SimulatedAuth.currentUser };
      }
      throw new Error("Invalid OTP");
    }};
  },
  signOut: async () => { SimulatedAuth.currentUser = null; }
};

// ========== AUTH SCREENS ==========
function AuthScreen({ dark, onLogin, onSkip }) {
  ...
  const handleSendOTP = async () => {
    if (phone.length !== 10) { setError("Valid 10-digit mobile number daalo"); return; }
    setLoading(true); setError("");
    try {
      // In real app: Firebase Phone Auth
      // const auth = getAuth(firebaseApp);
      // const conf = await signInWithPhoneNumber(auth, "+91" + phone, recaptchaVerifier);
      const conf = await SimulatedAuth.signInWithPhoneNumber("+91" + phone);
      setConfirmation(conf);
      setStep("otp");
      setSuccess("OTP bheja gaya +91 " + phone + " pe!");
      startResendTimer();
    } catch(e) {
  ...
  const handleVerifyOTP = async () => {
    ...
    setLoading(true); setError("");
    try {
      const result = await confirmation.confirm(otpStr);
      onLogin(result.user);
    } catch(e) {
      setError("Galat OTP hai. Preview mein use karo: 123456");
    }
    setLoading(false);
  };
  ...
  const handleOtpChange = (i, val) => {
    ...
    if (!val && i > 0 && otpRefs[i-1].current) otpRefs[i-1].current.focus();
  };
  ...
  return (
    <div className="auth-overlay">
      <div className="auth-box">
          ...
          {step === "phone" ? (
            <>
              ...
              <div className="auth-sub">
                Phone number daalo — OTP aayega. Koi password nahi, koi jhanjhat nahi!
              </div>
              ...
              <button className="auth-btn" onClick={handleSendOTP} disabled={loading || phone.length !== 10}>
                {loading ? "⏳ OTP bhej raha hoon..." : "📨 OTP Bhejo"}
              </button>
              <div className="auth-skip">
                Pehle try karna hai?{" "}
                <button onClick={onSkip}>Bina login ke dekho →</button>
              </div>
            </>
          ) : (
            <>
              <div className="auth-title">🔐 OTP Verify Karo</div>
              <div className="auth-sub">
                +91 {phone} pe OTP bheja gaya hai.<br/>
                <span style={{fontSize:11, color:'#6366f1'}}>Preview mein type karo: 1 2 3 4 5 6</span>
              </div>
              {error && <div className="auth-error">⚠️ {error}</div>}
              ...
              <button className="auth-btn" onClick={handleVerifyOTP} disabled={loading || otp.join("").length !== 6}>
                {loading ? "⏳ Verify ho raha hai..." : "✅ Verify Karo"}
              </button>
              <div className="auth-resend">
                {resendTimer > 0
                  ? `Resend OTP in ${resendTimer}s`
                  : <><button onClick={() => { setStep("phone"); setOtp(["","","","","",""]); setError(""); setSuccess(""); }}>← Number change karo</button> · <button onClick={handleSendOTP}>Resend OTP</button></>
                }
              </div>
            </>
          )}
        ...
        <div style={{textAlign:"center", marginTop:16, fontSize:11, color: dark?"#4b5563":"#9ca3af", fontFamily:"DM Mono, monospace"}}>
...
export default function App() {
  ...
  const handleUpgrade = (planType) => {
    ...
    const msg = planType === "yearly"
      ? "Yearly Pro unlock ho gaya! Enjoy karo!"
      : planType === "halfyear"
      ? "6 Month Pro unlock ho gaya! Enjoy karo!"
      : "Monthly Pro unlock ho gaya! Enjoy karo!";
    alert(msg);
  };
    ...
    <>
      ...
      <div className="app">
        ...
        <main className="main">
          ...
          {tab === "evaluator" && <PromptEvaluatorTab dark={dark} />}
          {tab === "mistakes" && (
