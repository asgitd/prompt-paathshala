...
const getStyles = (dark) => `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ai-response-body { padding: 14px; font-size: 13px; color: ${dark ? '#d1d5db' : '#374151'}; line-height: 1.75; white-space: pre-wrap; max-height: 320px; overflow-y: auto; }
...
async function callClaude(systemPrompt, userMessage) {
  const apiKey = "";  // Add your key in Vercel env vars
  if (!apiKey) {
    return "⚠️ API Key nahi mili. Vercel mein VITE_ANTHROPIC_API_KEY environment variable add karo.";
  }
  try {
    ...
    if (!response.ok) {
      ...
      return `❌ Error: ${err.error?.message || "Kuch gadbad hui. Dobara try karo."}`;
    }
    const data = await response.json();
    return data.content?.[0]?.text || "Response nahi mila. Dobara try karo.";
  } catch (e) {
    return "❌ Network error. Internet connection check karo aur dobara try karo.";
  }
}
      ...
      {(loading || response) && (
        <div className="ai-response">
          <div className="ai-response-header">
            <span>🤖</span>
            <span className="ai-response-label">Claude ka Live Response</span>
          </div>
          {loading ? (
...
function UpgradeModal({ onClose, onUpgrade }) {
  ...
  const PLANS = {
    ...
    lifetime: { price: 499, label: "₹499", name: "Lifetime",  desc: "ek baar · forever!",amount:49900, tag: "Forever ⭐",   popular: false },
  };
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-sub">Monthly ya Lifetime — tumhari marzi!</div>
    ...
    privacy: {
      ...
      content: (
        <>
          ...
          📧 Email: privacy@promptpaathshala.in<br/>
      ...
      content: (
        <>
          ...
          <ul>
            ...
            <li><strong>Lifetime Plan:</strong> ₹299 one-time payment, no recurring charges</li>
          ...
          <h2>11. Contact</h2>
          <p>📧 Email: legal@promptpaathshala.in</p>
        </>
      )
    },
    refund: {
      ...
      content: (
        <>
          ...
          <h2>2. Lifetime Plan (₹299 one-time)</h2>
          <ul>
            <li><strong>7-day refund window:</strong> Full refund if requested within 7 days of purchase</li>
            <li><strong>After 7 days:</strong> No refund (you retain lifetime access)</li>
          </ul>
          ...
          <p>Email us at <strong>refunds@promptpaathshala.in</strong> with:</p>
          <ul>
          ...
          <h2>7. Contact</h2>
          <p>📧 Email: refunds@promptpaathshala.in<br/>
    ...
    cookies: {
      ...
      content: (
        <>
          ...
          <h2>8. Contact</h2>
          <p>📧 Email: privacy@promptpaathshala.in</p>
        </>
      )
    }
  };
  ...
  return (
    <div className="legal-wrapper">
      ...
      <div className="legal-footer">
        Questions? Email us at legal@promptpaathshala.in · Prompt Paathshala © 2026
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const handleUpgrade = (planType) => {
    ...
    const msg = planType === "lifetime"
      ? "Lifetime Pro unlock ho gaya! Kabhi expire nahi hoga!"
          ...
          {tab === "learn" && (
            <>
              ...
              <div style={{textAlign:'center', marginTop:28, color: dark?'#374151':'#9ca3af', fontFamily:'DM Mono,monospace', fontSize:12}}>
                ▶ Live Test button dabao → real Claude response dekho ✦
              </div>
            </>
          )}
