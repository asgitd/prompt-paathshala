...
// Firebase Auth is used directly
...
function AuthScreen({ dark, onLogin }) {
  ...
  const initFB = async () => {
    try {
      ...
      return { auth: getAuth(app), createUserWithEmailAndPassword, signInWithEmailAndPassword, GoogleAuthProvider, signInWithPopup, sendEmailVerification };
    } catch(e) { return null; }
  };

  const handleEmail = async () => {
    if (!email.includes("@")) { setError("Valid email daalo"); return; }
    if (pass.length < 6) { setError("Password min 6 characters chahiye"); return; }
    setLoading(true); setError("");
    try {
      const fb = await initFB();
      if (!fb) throw new Error("Firebase connect nahi ho pa raha. Internet check karo.");
      if (isSignup) {
  ...
  const handleGoogle = async () => {
    ...
    try {
      const fb = await initFB();
      if (!fb) throw new Error("Firebase connect nahi ho pa raha.");
      const provider = new fb.GoogleAuthProvider();
      provider.setCustomParameters({ prompt: "select_account" });
      const result = await fb.signInWithPopup(fb.auth, provider);
      onLogin(result.user);
    } catch(e) {
      if (e.code !== "auth/popup-closed-by-user") setError("Google login error: " + (e.message || "Dobara try karo"));
    }
    setGLoading(false);
  };
      ...
      <div className="auth-box">
        ...
        <div className="auth-card">
          <div style={{textAlign:"center", padding:"10px 0"}}>
            ...
            <div style={{fontFamily:"Syne,sans-serif", fontWeight:800, fontSize:18, color:dark?"#e8e8f0":"#111118", marginBottom:10}}>Email Verify Karo!</div>
            <div style={{fontSize:13, color:dark?"#9ca3af":"#6b7280", lineHeight:1.8, marginBottom:18}}>
              <strong style={{color:"#a5b4fc"}}>{email}</strong> pe verification email bheja gaya hai.<br/>
              Email kholo → Link click karo → Wapas aao → Login karo!
            </div>
            <div style={{background:"rgba(99,102,241,0.08)", border:"1px solid rgba(99,102,241,0.2)", borderRadius:10, padding:"10px 12px", marginBottom:14, fontSize:12, color:"#9ca3af"}}>
  ...
  return (
    <div className="auth-overlay">
      <div className="auth-box">
        ...
        <div className="auth-card">
          {/* Login/Signup Toggle */}
          <div style={{display:"flex", gap:6, marginBottom:18, background:"rgba(0,0,0,0.2)", borderRadius:12, padding:4}}>
          ...
          <button className="auth-btn" onClick={handleEmail} disabled={loading || !email || pass.length < 6}>
    ...
    <>
      ...
      <div className="app">
        ...
        <header className="header">
          ...
          <div className="badge">🧠 India ka AI Prompt Engineering Course</div>
