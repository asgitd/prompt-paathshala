...
async function initFirebase() {
  ...
  try {
    ...
    const { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword,
            GoogleAuthProvider, signInWithPopup } = await import("https://www.gstatic.com/firebasejs/10.8.0/firebase-auth.js");
    const app = getApps().length === 0 ? initializeApp(FIREBASE_CONFIG) : getApps()[0];
    _fbInstance = { auth: getAuth(app), createUserWithEmailAndPassword, signInWithEmailAndPassword, GoogleAuthProvider, signInWithPopup };
    return _fbInstance;
  } catch(e) {
...
function AuthScreen({ dark, onLogin, onSkip }) {
  const [tab, setTab] = useState("email"); // email | signup
  ...
  const handleEmail = async () => {
    ...
    try {
      ...
      if (fb) {
        const fn = isSignup ? fb.createUserWithEmailAndPassword : fb.signInWithEmailAndPassword;
        const result = await fn(fb.auth, email, pass);
        onLogin(result.user);
      } else {
        // Preview fallback
        if (pass === "123456") onLogin({ email, uid: "preview_" + Date.now() });
