...
function checkRateLimit(key) {
  ...
  _apiCallLog[key] = _apiCallLog[key].filter(t => now - t < 60000); // last 1 min
  if (_apiCallLog[key].length >= 10) return false; // max 10 calls/min
...
async function callClaude(systemPrompt, userMessage) {
  ...
  for (const model of models) {
    try {
      ...
      if (result && !result.includes("high demand")) return result;
    } catch(e) {}
  }
...
async function callGemini(model, systemPrompt, userMessage) {
  if (!checkRateLimit("gemini")) {
    return "⚠️ Bahut zyada requests! Thoda wait karo (1 minute).";
  }
  const apiKey = "AIzaSyAnDFsXwrNKo7Cz-bkV2gzp94j8WZ5WpYc"; // Preview only - use env var in production
  if (!apiKey) {
    return "⚠️ API Key nahi mili. Vercel mein VITE_GEMINI_KEY environment variable add karo.";
  }
  // Sanitize inputs
  ...
  try {
    ...
    return data.candidates?.[0]?.content?.parts?.[0]?.text || "Response nahi mila. Dobara try karo.";
  } catch (e) {
    throw e;
  }
}
      ...
      {open && (
        <div className="tryit-body">
          ...
          {response && (
            ...
            <div style={{marginTop:10, padding:"10px 14px", borderRadius:10, background:"rgba(99,102,241,0.08)", border:"1px solid rgba(99,102,241,0.2)", display:"flex", alignItems:"center", gap:10, flexWrap:"wrap"}}>
              <span style={{fontSize:13}}>🧠</span>
              <span style={{fontSize:12, color:"#9ca3af", fontFamily:"DM Mono,monospace", flex:1}}>Aur detail mein samajhna hai?</span>
              <a href="https://claude.ai" target="_blank" rel="noopener noreferrer" style={{background:"linear-gradient(135deg,#4f46e5,#7c3aed)", color:"#fff", textDecoration:"none", fontFamily:"Syne,sans-serif", fontWeight:800, fontSize:11, padding:"5px 12px", borderRadius:8, whiteSpace:"nowrap"}}>Claude se puchho →</a>
            </div>
            </div>
          )}
        </div>
  ...
  return (
    <div className="lesson">
      ...
      {response && !loading && (
        <div style={{marginTop:10, padding:"10px 14px", borderRadius:10, background:"rgba(99,102,241,0.08)", border:"1px solid rgba(99,102,241,0.2)", display:"flex", alignItems:"center", gap:10, flexWrap:"wrap"}}>
          <span style={{fontSize:13}}>🧠</span>
          <span style={{fontSize:12, color:"#9ca3af", fontFamily:"DM Mono,monospace", flex:1}}>
            Aur detail mein samajhna hai?
          </span>
          <a href="https://claude.ai" target="_blank" rel="noopener noreferrer"
            style={{background:"linear-gradient(135deg,#4f46e5,#7c3aed)", color:"#fff", textDecoration:"none",
              fontFamily:"Syne,sans-serif", fontWeight:800, fontSize:11, padding:"5px 12px",
...
async function initFirebase() {
  ...
  try {
    ...
    _fbInstance = { auth: getAuth(app), createUserWithEmailAndPassword, signInWithEmailAndPassword, GoogleAuthProvider, signInWithPopup, sendEmailVerification };
    return _fbInstance;
  } catch(e) {
...
function AuthScreen({ dark, onLogin, onSkip }) {
  const [tab, setTab] = useState("email"); // email | signup | verify
      ...
      if (fb) {
        if (isSignup) {
          ...
          if (!result.user.emailVerified) {
            await fb.sendEmailVerification(result.user);
            await fb.auth.signOut();
            setError("Email verify nahi hua! Verification email dobara bheja gaya — inbox check karo.");
            setLoading(false);
            return;
          }
          onLogin(result.user);
        ...
        // Preview fallback
        if (pass === "123456") onLogin({ email, uid: "preview_" + Date.now(), emailVerified: true });
        else throw new Error("Preview mein password use karo: 123456");
      }
    } catch(e) {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      if (fb) {
        ...
        onLogin({ email: "preview@gmail.com", uid: "google_preview", displayName: "Preview User" });
      }
    } catch(e) {
        ...
        <div className="auth-card">
          ...
          {tab === "verify" && (
            <div style={{textAlign:"center", padding:"10px 0"}}>
              ...
              <button className="auth-btn" onClick={() => { setTab("email"); setError(""); }}
                style={{marginBottom:8}}>
                ✅ Email verify kar liya — Login karo
              </button>
              <button onClick={() => handleEmail()}
                style={{background:"none", border:"none", color:"#818cf8", cursor:"pointer", fontSize:12, fontFamily:"DM Mono,monospace", textDecoration:"underline"}}>
          ...
          <button onClick={handleGoogle} disabled={gLoading}
            ...
            {gLoading ? "⏳ Google se login ho raha hai..." : (
          ...
          {!isSignup && (
            <div style={{textAlign:"center", marginTop:10, fontSize:12, color:"#6b7280", fontFamily:"DM Mono,monospace"}}>
              Preview mein password: <strong style={{color:"#818cf8"}}>123456</strong>
            </div>
          )}

          {tab !== "verify" && (
            <div className="auth-skip">
              Pehle explore karna hai?{" "}
